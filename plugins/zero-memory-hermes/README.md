...
| `transform_tool_result` | After a `terminal` command, appends one line to its result when a squash commit's board card has no record of its landing — the only channel that reaches the model right after a command. Which commits count is the watcher's `landing` decision.                              |
