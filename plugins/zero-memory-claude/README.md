...
| `PostToolUse` hook      | Right after a `Bash` command, a squash commit whose board card has no record of its landing gets one line naming the exact `card` `land` call; each squash is checked once per machine (`zero-memory-watcher landing`).                                                                                                                  |
