#!/usr/bin/env bash
# deploy-zm-codex-remote-host.sh
#
...
# disk, then install the Codex integration from there. Run this ON the machine
# that uses Codex (the consumer). The Codex sibling of
# deploy-zm-claude-remote-host.sh.
#
# Why re-home for Codex: deploy-zm-codex.sh copies the plugin into ~/.codex and
# generates a local marketplace there, so the install leaves NO persistent
# pointer at the mount — but reading the ~90 MB binary + plugin straight off a
# flaky share DURING install is what fails. Copying to local disk first makes
...
$(say "Done. $CLIENT integration installed from local disk: $LOCAL_DEST")
  ...
  Next:
    ...
    • Restart Codex and TRUST the plugin hooks (Codex /hooks).
EOF
