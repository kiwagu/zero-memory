#!/usr/bin/env bash
# deploy-zm-hermes-remote-host.sh
#
...
# disk, then install the Hermes integration from there. Run this ON the machine
# that runs Hermes (the consumer). The Hermes sibling of
# deploy-zm-claude-remote-host.sh / deploy-zm-codex-remote-host.sh.
#
# Why re-home for Hermes: deploy-zm-hermes.sh copies the plugin into the Hermes
# profile and installs the binary to ~/.local/bin, so the install leaves NO
# persistent pointer at the mount — but reading the ~90 MB binary straight off
# a flaky share DURING install is what fails. Copying to local disk first makes
# the install read from stable storage.
#
# Usage:
#   bash deploy-zm-hermes-remote-host.sh [bundle-dir]
#
...
# Env: LOCAL_DEST (local copy destination), ZM_SERVER_URL (MCP endpoint;
# forwarded to the installer), HERMES_PROFILE (forwarded to the installer).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOCAL_DEST="${LOCAL_DEST:-$HOME/.local/share/zm-bundle}"
CLIENT="Hermes"
INSTALLER="deploy-zm-hermes.sh"
...
$(say "Done. $CLIENT integration installed from local disk: $LOCAL_DEST")
  ...
  Next:
    ...
    • RESTART Hermes so the plugin loads (plugins load at startup only),
      then check with /plugins and /zm status in a new session.
EOF
