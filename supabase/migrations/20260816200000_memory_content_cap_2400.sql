-- Migration: the hits' content cap moves from 1800 to 2400 characters
--
-- Purpose:
--   The section caps shipped with a measured curve of SIZE, and 1800 was
--   chosen on it. What that curve could not show — and what the id-based
--   holdout gate cannot see by construction, since it checks WHICH memories
--   arrive and never whether the part that arrived carries the point — is what
--   the cut actually removes. Measured afterwards on a clone of the live
--   corpus (2,105 live memories), counting rows whose TAIL opens an operative
--   section (the fix, the consequence, the invocation, what remains, what to
--   check) that the surviving HEAD does not already carry:
--
--     cap    rows truncated        operative half lost
--     1200   570 (27.1%)           89  = 4.2% of the corpus
--     1800   283 (13.4%)           31  = 1.5% of the corpus
--     2400    89  (4.2%)            9  = 0.4% of the corpus
--
--   Against the size curve for the same packs (1800 → 36,339 characters,
--   2400 → 37,978), moving to 2400 costs 4.5% of pack size and removes 3.7×
--   of the only harm this cap can do: delivering a fact without the part that
--   says what to do about it.
--
--   The trade is one-sided because the pack-size win never came from here. The
--   loop and recent caps carry it (they took a pack from 61,471 to 40,304 on
--   their own), and the hook channel is fitted by a budget that composes whole
--   sections regardless. What the hits' cap must do is bound the extreme — an
--   11,567-character row inlined whole — and 2400 still does that while
--   leaving a typical dense memory intact.
--
--   Same principle as the stub tail elsewhere in this pipeline: half a fact is
--   worse than a whole one plus a pointer. A cap that keeps the claim and cuts
--   the remedy is exactly the half-fact case.
--
-- Affected objects:
--   - function public.build_context (CREATE OR REPLACE; body only — the
--     default of the zm.memory_content_cap_chars knob, 1800 -> 2400). No
--     signature, grant, row-count or ranking change; an operator who has set
--     the knob explicitly is unaffected.
