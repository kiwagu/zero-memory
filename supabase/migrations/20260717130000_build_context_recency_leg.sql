-- Migration: recency leg + rules-promoted hard filter for briefings
--
-- Purpose:
--   A generic-topic session briefing returned an "encyclopedia": the
--   earliest self-descriptive facts always win topic similarity, so the
--   recent working set (this week's decisions and gotchas) never surfaced.
--   Briefings gain a `recent[]` section — the scope's freshest live
...
--   the ranked legs. Briefings also stop re-delivering memories whose
--   content was promoted into an always-on rules file: those arrive with
--   the rules layer already, so the pack copy is a duplicate delivery
--   (the reinforcement demotion already sinks them softly; a briefing
--   applies a hard filter).
--
-- Affected objects:
--   - function public.build_context: DROPPED and recreated with a new
--     trailing `briefing boolean default false` parameter (CREATE OR
--     REPLACE cannot add parameters without leaving an ambiguous overload
--     behind). Existing callers that do not pass `briefing` see the exact
--     pre-migration behavior; the pack gains an always-present additive
--     `recent` key (empty for non-briefing calls).
--
-- Special considerations:
--   - `briefing => true` does two things: fills `recent[]` and hard-filters
--     rules-promoted memories out of every knowledge leg. Mid-session
--     build_context calls stay untouched: a promoted memory is still
--     directly recallable, it just is not re-DELIVERED by a briefing.
--   - rule_candidates is owner-readable under RLS ("owners read their rule
--     candidates"), so the security-invoker filter sees exactly the
--     caller's own promotions.
--   - Two more instance knobs (current_setting, missing_ok):
--     zm.recent_window_days (default 14) and zm.recent_max (default 5).
--     The per-kind whitelist is deliberate: decisions/gotchas/conventions/
--     facts are working-set knowledge; episodes and open-loop kinds have
--     their own channels.
--   - Grants are re-applied verbatim after the drop (authenticated +
--     service_role for the eval harness).

set search_path = public, extensions;

drop function if exists public.build_context(
  extensions.vector, text, extensions.ltree[], int, int
);

create function public.build_context(
...
as $$
  with
    ...
    recent as (
      ...
      -- changed since the last visit"). Deduplicated against both ranked
      -- legs; rules-promoted memories excluded like everywhere else in a
      -- briefing.
      select
        memories.id,
        memories.content,
...
comment on function public.build_context(
  ...
  'decisions/gotchas/conventions/facts (zm.recent_window_days default 14, '
  'zm.recent_max default 5) independent of topic similarity, deduplicated '
  'against the ranked legs — and a hard filter keeping rules-promoted '
  'memories out of the pack (they already arrive via the rules layer); and '
  'the scopes'' ACTIVE open loops (oldest first, cap 10 + total, soft TTL '
  'zm.open_loop_ttl_days default 14). Security invoker: RLS applies '
  'throughout.';

-- Grants: re-applied after the drop (authenticated callers + the eval
-- harness's service-role replay).
