-- Migration: add write_tokens_saved to dashboard_metrics
--
-- Purpose:
--   The "Tokens saved" tile splits into a Read side (context ZM injected via
--   briefings — the existing saved_tokens) and a Write side (facts ZM wrote for
--   you — the token size of agent-captured memory content). Both in tokens so
--   the user can sum them into a total "cost to rebuild". Write tokens are a
--   char/4 estimate of memory content, symmetric with the read side's own
--   row-heuristic sizing; labelled as an estimate in the UI.
--
-- Affected objects:
--   - function: public.dashboard_metrics(integer) -> jsonb (+write_tokens_saved)
