-- Migration: typed-link neighbours get a reserved slice of the linked leg
--
-- Purpose:
--   A memory is stored as an atomic fact; the connective tissue that gives it
--   meaning — what superseded it, what it contradicts, what it was derived
--   from — sits in public.memory_links and never reaches a briefing. The
--   linked leg exists but selects on ENTITY co-mention only, so two memories
--   the author explicitly linked do not qualify while two that happen to name
--   the same entity do.
--
--   THE CONSTRAINT: the pack must not grow. Enlarging a briefing is not a
--   reliable way to deliver more (measured here: packs already overflow their
--   delivery channel), so neighbours cannot be appended — they must displace.
--
--   HOW, and why not the obvious way. The first attempt scored a neighbour as
--   "parent hit score × relation weight" and let it compete inside the leg.
--   Measured on a clone of the live corpus, that delivers NOTHING: hit scores
--   come from the RRF fusion (min 0.019, avg 0.029, max 0.038) while the
--   linked leg scores with the similarity family (min 0.775, avg 0.838), so a
--   neighbour's best possible score is ~4% of the weakest row it must beat —
--   0 of 669 neighbour edges entered the leg across 25 probes. The two scores
--   are not on one scale and no weighting fixes that.
--
--   So relations get a RESERVED SLICE of the same cap instead: up to
--   `zm.graph_reserved_rows` of the leg's `max_memories` rows are filled from
--   the graph, ranked among THEMSELVES (where the shared scale holds), and the
--   entity leg fills what remains. The leg emits exactly as many rows as
--   before — the reserved rows displace entity co-mentions rather than adding
--   to them — and an unused reservation is given back, so a corpus with no
--   links produces the pack it always did.
--
--   WHAT THE RESERVATION WILL ACTUALLY CARRY, measured on the same corpus so
--   nobody expects otherwise: of the neighbour edges touching hits, 94% of the
--   eligible ones are `relates_to`. `supersedes` is nearly empty by
--   construction — 167 of its 169 edges point at a RETIRED memory, since
--   superseding invalidates the old version and hits are the survivors — and
--   `contradicts` / `derived_from` are rare (7 and 6 eligible). The reservation
--   therefore mostly buys author-declared association; whether that is worth
--   displacing a top-similarity row is exactly what the displacement gate
--   measures, and the knob goes to 0 if it is not.
--
...
--
-- Affected objects:
--   - function public.build_context (CREATE OR REPLACE; body only — new knobs,
--     three added CTEs and the composition of the linked leg; signature,
--     grants, the leg's ROW COUNT and every other leg unchanged).
--
-- Knobs (all optional; the content caps come from the previous migration):
--   zm.memory_content_cap_chars  integer, default 1800
--   zm.loop_content_cap_chars    integer, default 400
--   zm.recent_content_cap_chars  integer, default 400
--   zm.graph_reserved_rows       integer, default 2   (0 disables the feature)
--   zm.graph_neighbors_per_hit   integer, default 2
--   zm.graph_weight_supersedes   double,  default 0.9
--   zm.graph_weight_contradicts  double,  default 0.9
--   zm.graph_weight_derived_from double,  default 0.7
--   zm.graph_weight_relates_to   double,  default 0.5
...
comment on function public.build_context(
  ...
  'family as the hits, plus up to zm.graph_reserved_rows rows claimed by '
  'typed-link neighbours of the hits (both directions, fan-out '
  'zm.graph_neighbors_per_hit per hit, relation weights '
  'zm.graph_weight_*) which DISPLACE entity co-mentions rather than '
  'enlarging the leg, an unused reservation returning to the entity leg; '
  'EVERY section inlines content up '
