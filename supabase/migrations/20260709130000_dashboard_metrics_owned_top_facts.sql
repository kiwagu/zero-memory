-- Migration: restrict dashboard_metrics top_facts to the caller's own memories
--
-- Purpose:
--   Privacy fix: "Facts that fired most" must never surface another
--   user's memory — not even its id or a truncated placeholder, which would
--   leak the existence (and partial content) of a memory the caller does not
--   own. The prior definition LEFT JOINed memories, so a foreign id surfaced in
--   a shared-scope recall appeared with null content. This switches to an INNER
--   JOIN on ownership, dropping foreign ids from the ranking entirely. Team-admin
--   cross-visibility is a separate, later feature.
--
-- Affected objects:
--   - function: public.dashboard_metrics(integer) -> jsonb (top_facts only)
...
begin
  ...
  -- Top surfaced facts, OWNED ONLY: the inner join drops any surfaced id the
  -- caller does not own, so no foreign memory (or its existence) is disclosed.
